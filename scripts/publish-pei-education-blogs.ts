...
function authHeader(): string {
  if (process.env.SYNC_API_TOKEN) return `Bearer ${process.env.SYNC_API_TOKEN}`;
  const email = process.env.ADMIN_EMAIL;
  const password = process.env.ADMIN_PASSWORD;
  if (!email || !password) {
    throw new Error("Set SYNC_API_TOKEN or ADMIN_EMAIL + ADMIN_PASSWORD.");
  }
...
async function postJson(path: string, body: unknown): Promise<string> {
  const baseUrl = (process.env.REMOTE_SYNC_URL ?? SITE_URL).replace(/\/+$/, "");
...
async function main() {
  const postPayload = posts.map((post) => ({
    slug: post.slug,
    title: post.title,
    ...
    noIndex: false,
    readingTime: readingTime(post.html),
    featured: false,
    authorEmail: process.env.POST_AUTHOR_EMAIL ?? process.env.ADMIN_EMAIL ?? null,
  ...
  console.log(await postJson("/api/admin/sync/taxonomy", { categories: [CATEGORY], tags: TAGS }));
  console.log(await postJson("/api/admin/sync/posts", { posts: postPayload }));
  console.log(`Published ${postPayload.length} PEI education blog posts.`);
}
