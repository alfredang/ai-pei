/**
 * Pure (sharp-free) SVG builder for a blog post's branded cover. Kept separate
 * from post-cover.ts so server components (the /blog list + /blog/[slug] detail)
 * can render the SAME card inline as a fallback when a post has no stored
 * featuredImage — without bundling the native `sharp` dependency.
 *
 * See .claude/skills/blog-cover-image/SKILL.md for the rules every change here
 * must respect. Font family is plain sans-serif because sharp/libvips's SVG
 * renderer does NOT load webfonts; keeping it identical here means the inline
 * fallback matches the R2-stored PNG covers pixel-for-pixel.
 */
